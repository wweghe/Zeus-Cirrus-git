...
/**
   \file
\anchor core_rest_get_alloc_scheme

   \brief   Retrieve Allocation Scheme instance(s) registered in SAS Risk Cirrus Objects
   ...
   \param [in] filter Filters to apply on the GET request when no value for key is specified. Example: request GET /allocationSchemes?name=Data1|Data2&statusCd=Draft
   ...
   \param [out] outds Name of the output table that contains the Allocation Scheme instances (Default: alloc_scheme_summary)
   \param [out] outds_details Name of the output table that contains the Allocation Scheme spreadsheet data (Default: alloc_scheme_details)
   \param [out] outVarToken Name of the output macro variable which will contain the access token (Default: accessToken)
   \param [out] outSuccess Name of the output macro variable that indicates if the request was successful (&outSuccess = 1) or not (&outSuccess = 0). (Default: httpSuccess)
   \param [out] outResponseStatus Name of the output macro variable containing the HTTP response header status: i.e. HTTP/1.1 200 OK. (Default: responseStatus)

   \details
   This macro sends a GET request to <b><i>\<host\>:\<port\>/riskCirrusObjects/objects/allocationSchemes</i></b> and collects the results in the output tables. \n
   ...
   2) Send a Http GET request and parse the JSON response into the output table work.alloc_scheme_summary and work.alloc_scheme_details
   \code
      %let accessToken =;
      %core_rest_get_alloc_scheme(key = f5cbf66a-1b06-4df7-914e-f108ae9c80a8
                                 , outds = work.alloc_scheme_summary
   ...
   \date    2023
*/
%macro core_rest_get_alloc_scheme(host =
                                 , port =
   ...
   %let requestUrl = &baseUrl./&server./objects/allocationSchemes?includeFields=objectLinks;
   ...
   /* Get the Allocation Scheme link types, if needed */
   %if(%sysevalf(%superq(inDsLinkTypes) eq, boolean)) %then %do;

      %let link_types_ds=work.alloc_scheme_link_types;
      ...
      %core_rest_get_link_types(host = &host.
                                 ...
                                 , filter = in(objectId,%27allocationScheme_bep%27,%27allocationScheme_defaultModel%27,%27allocationScheme_model%27,%27allocationScheme_cycle%27,%27allocationScheme_analysisRun%27)
                                 , outds = &link_types_ds.
      ...
      %if(not &&&outSuccess.. or not %rsk_dsexist(alloc_scheme_link_types)) %then %do;
         %put ERROR: Unable to get the Allocation Scheme link types;
         %abort;
      %end;
   ...
   %core_rest_request(url = &requestUrl.
                     ...
                     , client_id = &client_id.
                     , client_secret = &client_secret.
                     , parser = sas.risk.cirrus.core_rest_parser.coreRestAllocationScheme
                     , outds = &outds.
                     , arg1 = &link_types_ds.
                     , arg2 = &outds_details.
                     , outVarToken = &outVarToken.
   ...
   %if(not &&&outSuccess.. or not %rsk_dsexist(&outds.)) %then %do;
      %put ERROR: Unable to get the allocation scheme;
      %abort;
   %end;
