...
\anchor core_apply_allocation_rules
   ...
   \param [out] outCasLib Name of the library to which the data will be passed. If defined will be used instead of 'work' libname of output data.
   ...
   | ADJUSTMENT_VALUE                      | NUMERIC(8)      | Y          | Adjustment Value          | The value of the adjustment. It can be absolute, relative percentage (i.e. as a percentage increase/decrease) or relative increment (additive adjustment).  For Classification rules, this field will be character(150). |
   ...
   |-------------------|--------------------|---------------------------|------------------|-------------------------------------------------------|---------------------------------------------------------------------|-------------------------------------------------------------       |
   | EVEN              | SUM                | Y                         | Y                | \f[v'_n = \frac{Adj}{\sum_n w_n}\f]                   | \f[v'_n = \frac{T \cdot (1 + Adj)}{\sum_n w_n}\f]                   | \f[v'_n = v_n + frac{Adj}{N}\f]                                    |
   | EVEN              | SUM                | Y                         | N                | \f[v'_n = \frac{Adj}{\sum_n w_n}\f]                   | \f[v'_n = \frac{T \cdot (1 + Adj)}{\sum_n w_n}\f]                   | GENERATES ERROR due to missing variable                            |
   | EVEN              | SUM                | N                         | *                | \f[v'_n = \frac{Adj}{N}\f]                            | \f[v'_n = \frac{T \cdot (1 + Adj)}{N}\f]                            | \f[v'_n = v_n + frac{Adj}{N}\f]                                    |
   
   | EVEN              | MEAN               | Y                         | Y                | \f[v'_n = Adj\f]                                      | \f[v'_n = T \cdot (1 + Adj)\f]                                      | \f[v'_n = v_n + Adj\f]                                             |
   | EVEN              | MEAN               | Y                         | N                | \f[v'_n = Adj\f]                                      | \f[v'_n = T \cdot (1 + Adj)\f]                                      | GENERATES ERROR due to missing variable                            |
   | EVEN              | MEAN               | N                         | *                | \f[v'_n = Adj\f]                                      | \f[v'_n = T \cdot (1 + Adj)\f]                                      | \f[v'_n = v_n + Adj\f]                                             |
   
   | PROPORTIONAL      | SUM                | N                         | *                | \f[v'_n = Adj \cdot \frac{w_n}{\sum_n w_n}\f]         | \f[v'_n = \frac{T \cdot (1 + Adj) \cdot w_n}{\sum_n w_n}\f]         | \f[v'_n = v_n + ( Adj \cdot \frac{v_n}{T})\f]                      |
   | PROPORTIONAL      | SUM                | Y                         | N                | \f[v'_n = \frac{Adj \cdot v_n}{T}\f]                  | \f[v'_n = v_n \cdot (1 + Adj)\f]                                    | GENERATES ERROR due to missing variable                            |
   | PROPORTIONAL      | SUM                | Y                         | Y                | \f[v'_n = \frac{Adj \cdot v_n}{T}\f]                  | \f[v'_n = v_n \cdot (1 + Adj)\f]                                    | \f[v'_n = v_n + ( Adj \cdot \frac{w_n}{\sum_n w_n})\f]             |
   
   | PROPORTIONAL      | MEAN               | N                         | *                | \f[v'_n = \frac{Adj \cdot N \cdot w_n}{\sum_n w_n}\f] | \f[v'_n = \frac{T \cdot (1 + Adj) \cdot N \cdot w_n}{\sum_n w_n}\f] | \f[v'_n =  v_n + (Adj \cdot N \cdot (\frac{v_n}{T \cdot T * N}) )  |  
   | PROPORTIONAL      | MEAN               | Y                         | N                | \f[v'_n = \frac{Adj \cdot v_n}{T}\f]                  | \f[v'_n = v_n \cdot (1 + Adj)\f]                                    | GENERATES ERROR due to missing variable                            |
   | PROPORTIONAL      | MEAN               | Y                         | Y                | \f[v'_n = \frac{Adj \cdot v_n}{T}\f]                  | \f[v'_n = v_n \cdot (1 + Adj)\f]                                    | \f[v'_n = v_n + (\frac {Adj \cdot N \cdot w_n}{\sum_n w_n}  )      |
...
%macro core_apply_allocation_rules(ds_in =
            ...
            %do i = 1 %to &tot_rules.;
               %if(&&allocation_method_&i. ne INDIVIDUAL) %then %do;
                  ...
                  %if(&&weighted_aggr_flg_&i.. = Y) %then %do;
                     %if(&&aggr_method_&i.. = SUM) %then
                        /* Perform weighted sum aggregation */
                        , sum(&&measure_col_&i.. * max(&&weight_col_&i..,0) * (&&where_clause_&i..)) format = best32.;
                     %else %if(&&aggr_method_&i.. = MEAN) %then
                        /* Perform weighted average aggregation */
                        , sum(&&measure_col_&i.. * max(&&weight_col_&i..,0) * (&&where_clause_&i..)) / sum(max(&&weight_col_&i..,0) * (&&where_clause_&i..)) format = best32.;
                  ...
                  %else %do;
                     %if(&&aggr_method_&i.. = SUM) %then
                         /* Perform weighted average aggregation */
                        , &&aggr_method_&i..(max(&&measure_col_&i..,0) * ifn(&&where_clause_&i.., 1, .)) format = best32.;
                     %else %if(&&aggr_method_&i.. = MEAN) %then
                        /* Perform weighted average aggregation */
                        , &&aggr_method_&i..(max(&&measure_col_&i..,0) * ifn(&&where_clause_&i.., 1, .)) format = best32.;
                     %else /* &&aggr_method_&i.. in (MAX, MIN) */
                  ...
                  %if(%sysevalf(%superq(weight_col_&i.) ne, boolean)) %then
                     , sum(max(&&weight_col_&i..,0) * (&&where_clause_&i..)) format = best32.;
   ...
   data
      ...
      %if(&audit_flg. = Y) %then %do;
         &outLibref..&ds_out_audit.(&audit_drop_stmt.
         ...
         if(&&where_clause_&i..) then do;
            ...
            %if(&&allocation_method_&i.. = EVEN) %then %do;
               ...
               %else %if(&&adjustment_type_&i.. = INCREMENT) %then %do;
                  /* SUM aggregation */
                  %if(&&aggr_method_&i.. = SUM) %then %do;
                        /* Even allocation, Incremental adjustment, Simple SUM aggregation */  
                        &&measure_col_&i.. = sum(&&measure_col_&i., (&&adj_value_&i. / &&current_value_cnt_&i.));
                  %end;
                  /* MEAN aggregation */
                  %else %if(&&aggr_method_&i.. = MEAN) %then %do;
                     /* Even allocation, Incremental adjustment, MEAN aggregation */
            ...
            %else %if (&&allocation_method_&i.. = PROPORTIONAL) %then %do;
               ...
               %if(&&adjustment_type_&i.. = RELATIVE) %then %do;
                  ...
                  %if(&&aggr_method_&i.. = SUM) %then %do;
                     %if(%sysevalf(%superq(weight_col_&i.) ne, boolean) and &&weighted_aggr_flg_&i.. = N) %then
                        ...
                        &&measure_col_&i.. = &&current_value_aggr_&i. * (1 + &&adj_value_&i.) * max(&&weight_col_&i..,0) / &&current_weight_sum_&i.;
                     %else
                  ...
                  %else %if(&&aggr_method_&i.. = MEAN) %then %do;
                     %if(%sysevalf(%superq(weight_col_&i.) ne, boolean) and &&weighted_aggr_flg_&i.. = N) %then
                        ...
                        &&measure_col_&i.. = &&current_value_aggr_&i. * (1 + &&adj_value_&i.) * &&current_value_cnt_&i.. * max(&&weight_col_&i..,0) / &&current_weight_sum_&i.;
                     %else
               ...
               %else %if(&&adjustment_type_&i.. = INCREMENT) %then %do;
                  ...
                  %if(&&aggr_method_&i.. = SUM) %then %do;
                     %if(&&weighted_aggr_flg_&i.. = Y) %then
                        /* Proportional allocation, Incremental adjustment, Weighted SUM aggregation. Allocation is proportional to the weight column. */
                        &&measure_col_&i.. = sum(&&measure_col_&i., (&&adj_value_&i. * max(&&weight_col_&i..,0) / &&current_weight_sum_&i.));
                     %else
                        /* Proportional allocation, Incremental adjustment, SUM aggregation. Allocation is proportional to the measure column. */
                        &&measure_col_&i.. = sum(&&measure_col_&i., (&&adj_value_&i. * max(&&measure_col_&i.,0) / &&current_value_aggr_&i.));
                     ;
                  %end;
                  /* MEAN aggregation */
                  %else %if(&&aggr_method_&i.. = MEAN) %then %do;
                     %if(&&weighted_aggr_flg_&i.. = Y) %then
                        /* Proportional allocation, Incremental adjustment, Weighted MEAN aggregation. Allocation is proportional to the weight column. */
                        &&measure_col_&i.. = sum(&&measure_col_&i., (&&adj_value_&i. * &&current_value_cnt_&i. * max(&&weight_col_&i..,0) / &&current_weight_sum_&i.));                        
                     %else
                        /* Proportional allocation, Incremental adjustment, Weighted MEAN aggregation. Allocation is proportional to the measure column. */
                        &&measure_col_&i.. = sum(&&measure_col_&i., (&&adj_value_&i. * &&current_value_cnt_&i. * max(&&measure_col_&i.,0) / (&&current_value_aggr_&i. * &&current_value_cnt_&i.)));
                     ;
                  %end;
                  /* MAX aggregation */
                  %else %if(&&aggr_method_&i.. = MAX) %then %do;
               ...
               %else %do;
                  ...
                  %if(&&aggr_method_&i.. = SUM) %then %do;
                     %if(%sysevalf(%superq(weight_col_&i.) ne, boolean) and &&weighted_aggr_flg_&i.. = N) %then
                        ...
                        &&measure_col_&i.. = &&adj_value_&i. * max(&&weight_col_&i..,0) / &&current_weight_sum_&i.;
                     %else
                  ...
                  %else %if(&&aggr_method_&i.. = MEAN) %then %do;
                     %if(%sysevalf(%superq(weight_col_&i.) ne, boolean) and &&weighted_aggr_flg_&i.. = N) %then
                        ...
                        &&measure_col_&i.. = &&adj_value_&i. * &&current_value_cnt_&i.. * max(&&weight_col_&i..,0) / &&current_weight_sum_&i.;
                     %else
            ...
            %if(&audit_flg. = Y) %then %do;
               ...
               %if(%core_get_vartype(&ds_in., &&measure_col_&i..) = N) %then %do;
                  ...
                  %if(&&weighted_aggr_flg_&i.. = Y) %then %do;
                     ...
                     __weight_value__ = max(&&weight_col_&i..,0);
                  %end;
               %end;
               %else
   ...
   data &outLibref..&ds_out_rule_summary.;
      length
