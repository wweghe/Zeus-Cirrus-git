...
\anchor corew_data_adjustments
    ...
    \param [in] rset_out_summary Name of the output table data contains the summary rules to be applied (Default: _tmp_data_rule_set_sum_).
    \param [in] rset_out_details Name of the output table data contains the detail rules to be applied per each input row (Default: _tmp_data_rule_set_sum_).
    ...
    \param [out] outCasLib Name of CAS lib for the output table. (Default: Public)
    ...
    \code
        %let accessToken =;
        %corew_data_adjustments(ds_in_data_adj_config = data_adj_config
                                , base_dttm =
...
%macro corew_data_adjustments(ds_in_data_adj_config =
                            , base_dttm =
                            , rset_out_summary = _tmp_data_rule_set_sum_
                            , rset_out_details = _tmp_data_rule_set_details_
                            , target_table_nm = ds_out
    ...
    %do dadj = 1 %to &TotRuns.;
        ...
        data ruleset_data_&dadj.;
            ...
            source_table = "&outCasLibref..ado_&dadj._&asOfDateFmtAd._&casTablesTag";
            source_table_desc = "&&analysis_data_name_&dadj.. (%left(&&analysis_data_key_&dadj..))";
            target_table = "&outCasLibref..ado_&dadj._&asOfDateFmtAd._&casTablesTag.";
        run;
        ...
        %if ("&&ruleSet_type_&dadj." eq "BUSINESS_RULES") %then %do;
            ...
            /* TODO: tables layout to be revised once all the data definitions for all AdjTypes were defined */
            %if &dadj. eq 1 %then %do;
                /* Create output tables structure */
                data
                    &rset_out_summary. (drop = rule_primary_key)
                    &rset_out_details. (drop = rule_weight rule_match_cnt total_row_cnt)
                    ;
                    attrib
            ...
            data work.data_adj_summary_&dadj.;
                set &rset_out_summary.;
                stop;
            run;
            data work.data_adj_details_&dadj.;
                set &rset_out_details.;
                stop;
            run;
            ...
            %if(&keep_sum_detail_data_flg. = Y) %then %do;
                ...
                data data_adj_summary_view_&dadj. / view = data_adj_summary_view_&dadj.;
                    attrib
                    cycle_id             length = $100.     label = "Cycle ID"
                    cycle_name           length = $256.     label = "Cycle Name"
                ...
                /* Set additional columns in the output */
                data data_adj_details_view_&dadj. / view = data_adj_details_view_&dadj.;
                    attrib
                    cycle_id             length = $100.     label = "Cycle ID"
                    cycle_name           length = $256.     label = "Cycle Name"
                    analysis_run_id      length = $40.      label = "Analysis Run ID"
                ...
                proc append data = data_adj_summary_view_&dadj.
                            base = &rset_out_summary.
                            force;
                run;

                /* Append info about the created analysis data object to the output table */
                proc append data = data_adj_details_view_&dadj.
                            base = &rset_out_details.
                            force;
                run;
        ...
        %if("&&ruleSet_type_&dadj." eq "ALLOCATION_RULES" or "&&ruleSet_type_&dadj." eq "QFACTOR_RULES" or "&&ruleSet_type_&dadj." eq "MGTACTION_RULES") %then %do;
            ...
            %if &dadj. eq 1 %then %do;
                data 
                    &rset_out_summary. (drop = rule_primary_key)
                    &rset_out_details. (drop = total_row_cnt);
                    attrib
                        source_table               length = $100.     label = "Source Table"
                        source_table_desc          length = $256.     label = "Source Table Description"
                        execution_dttm             length = 8.        label = "Execution Datetime"         format = datetime21.
            ...
            %core_apply_allocation_rules(ds_in = &schema_name.
                                        , rule_def_ds = ruleset_data_&dadj.
                                        , exclude_filter_vars = ruleSetKey
                                        , custom_filter_var = filter_exp
                                        , ds_out = modified_details_&dadj.
                                        , ds_out_rule_summary = data_adj_summary_&dadj.
                                        , ds_out_audit = data_adj_details_&dadj.
                                        );

            /* Append info about the created analysis data object to the output table */
            proc append data = data_adj_summary_&dadj.
                        base = &rset_out_summary.
                        force;
            run;

            /* Append info about the created analysis data object to the output table */
            proc append data = data_adj_details_&dadj.
                        base = &rset_out_details.
                        force;
            run;    
        ...
        %if("&&ruleSet_type_&dadj." eq "CLASSIFICATION_RULES" ) %then %do;
            ...
            %if &dadj. eq 1 %then %do;
                data 
                    &rset_out_summary. (drop = rule_primary_key /* TODO: to update*/)
                    &rset_out_details. (drop = total_row_cnt /* TODO: to update*/);
                    attrib
