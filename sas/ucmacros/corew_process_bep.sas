%macro corew_process_bep(solution =
   ...
   %local   outCasLibref
            ...
            bepKey bep_name bep_interval_type bepSegVars
            synthSchemaName synthSchemaVersion found_configuration_flag;
   ...
   data _null_;
      ...
      call symputx("bep_interval_type", intervalType, "L");
   run;
   ...
   proc sql noprint;
      select distinct %sysfunc(ifc(&forecastTimeFlag.,  catt('"', name, '"'), catt('"', scenarioName, '"')))
         into :scenario_group_list separated by ' '
      from &outCasLibref.."&inScenarioSet."n
      ;
   quit;

   /* Filter the BEP details to the target variable and get a space-separated list of that target variable's segmentation variables */
   data &ds_out_bep_details.;
      length scenario_group_name $200.;
      set bep_details (where=(upcase(targetVar)=upcase("&targetVar.")));
      if _n_=1 then
         call symputx("bepSegVars", strip(catx(" ", segVar1, segVar2, segVar3, segVar4, segVar5)), "L");

      if strip(scenario_group_name)="" then do;
         %do s=1 %to %sysfunc(countw(%bquote(&scenario_group_list.), " ", q));
            scenario_group_name=%scan(%bquote(&scenario_group_list.), &s., " ", q); output;
         %end;
      end;
      else
         output;
   ...
   */
   %if &gen_table_exists. or &elim_table_exists. %then %do;
      ...
      /* Add/update the _inst_scenario_name_ values to their actual scenario names for this run.
         Note: If scenario_name is blank, it is mapped to all scenarios in the scenario sets.
      The update synthetic/eliminated positions and the scenario data are available to the postGen code */
      data &outCasLibref..scenario_set_map (keep=scenarioName _inst_scenario_name_ _inst_scenario_forecast_time_);
         set &outCasLibref.."&inScenarioSet."n end=last;
         rename synthetic_scenario_name=_inst_scenario_name_ forecast_time=_inst_scenario_forecast_time_;
         if strip(synthetic_scenario_name)="" then
            synthetic_scenario_name=strip(ifc(&forecastTimeFlag., name, scenarioName));
      run;
      ...
      data  &outCasLibref..&outGeneratedPositions. (drop=__rc__ scenarioName _inst_scenario_name_orig_ _inst_scenario_ft_orig_ i _lastEffectiveDate_)
         ...
         length _inst_scenario_name_orig_ varchar(200) _inst_scenario_ft_orig_ 8. synthetic_instrument_flg varchar(3);
         ...
         do while(__rc__ eq 0);
            ...
            if gen then do;
               ...
               _effectiveDate_ = intnx("&bep_interval_type.", &asOfDate., _inst_scenario_forecast_time_, "S");
               ...
               %if &forecastTimeFlag. %then %do;
                  _lastEffectiveDate_ = intnx("&bep_interval_type.", &asOfDate., _inst_scenario_forecast_time_-1, "S");
               %end;
   ...
   /* If the portDataDefKey is provided, build the synthetic positions data definition to be similar to the datastore_config entry
   for that data definition.  If not provided/not found, create a generic synthetic positions data definition. */
   %let found_configuration_flag=0;
   %if (%sysevalf(%superq(portDataDefKey) ne, boolean)) %then %do;
      ...
      data _null_;
         ...
         call symputx("synthSchemaName", schemaName, "L");
         call symputx("synthSchemaVersion", schemaVersion, "L");
      run;

      data datastore_config;
         set datastore_config (where=(schemaName="&synthSchemaName." and schemaVersion="&synthSchemaVersion."));
   ...
   data work.datastore_config_override;
      ...
      schemaName="ST_BEP_SYNTH_POSITION";
      schemaVersion=coalescec(schemaVersion, "%sysget(SAS_RISK_CIRRUS_CADENCE)");
      sourceLibref="&outCasLib.";
      targetTableName="&outGeneratedPositions.";
      dataDefinitionName="BEP Synthetic Instrument Definition";
      dataDefinitionDesc="BEP Synthetic Instrument Definition";
