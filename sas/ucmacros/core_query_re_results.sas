%macro core_query_re_results(envTableCasLib =
   ...
   */
   ...
   /* Resolve the horizon list */
   %core_resolve_re_horizons(inHorizonsList = %bquote(&horizons.)
                           , outHorizonsVar = horizonsResolved
                           , inScenariosCasLib = &statesLib.
                           , inScenariosTable = &statesTable.
                           , horizonVarName = _horizon_
   ...
   %let outHorizons=;
   %if "&horizonsResolved." ne "ALL" or "&scenarios" ne "" %then %do;

      /* Determine the horizon index query */
      %let horizonIndex = {", max(_horizon_), "};
      %if "&horizonsResolved." ne "ALL" %then %do;
         %let horizonIndex = { &horizonsResolved. };
      %end;
      ...
      proc sql noprint;
         ...
         %if "&horizonsResolved." eq "ALL" %then %do;
         ...
         outputs={
            {
               ...
               table={
                  ...
                  %else %do;
                     name="&outResults."
                  %end;
                  %if (%sysevalf(%superq(outputTableWhereClause) ne, boolean)) %then %do;
                     where={"&outputTableWhereClause."}
                  %end;
               }
            }
         }
