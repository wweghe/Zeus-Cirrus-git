   ...
   %if(%sysevalf(%superq(wait_flg) = Y, boolean)) %then %do;
      ...
      %if "&execution_status." = "completed" %then %do;
         ...
         %if(%sysevalf(%superq(outEnvTableInfo) ne, boolean)) %then %do;
            ...
            %let envTableUrl = &requestUrl./&rePipelineKey./nodes/saveEnvironment/results/environment;

            /* Get the environment table details */
            /* Temporary disable mlogic and symbolgen options to avoid printing of userid/pwd to the log */
            option nomlogic nosymbolgen;
            /* Send the REST request */
            %core_rest_request(url = &envTableUrl.
                              , method = GET
                              , logonHost = &logonHost.
                              , logonPort = &logonPort.
                              , username = &username.
            ...
            /* Throw an error if the table request fails */
            %if(not &&&outSuccess..) %then %do;
               %put ERROR:Failed to retrieve the SAS Risk Engine pipeline environment table info from &envTableUrl. (Pipeline ID= &rePipelineKey.);
               %abort;
            %end;
            ...
            %if (not %rsk_dsexist(&outEnvTableInfo.) or %rsk_attrn(&outEnvTableInfo., nobs) eq 0) %then %do;
               %put ERROR: No information was found for the SAS Risk Engine pipeline environment table from &envTableUrl. (Pipeline ID= &rePipelineKey.);
               %abort;
            %end;
