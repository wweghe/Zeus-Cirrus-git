...
\anchor corew_synth_sample_generation
   ...
   \param [in] scenario_var Name of the scenario variable in ds_in_projection and ds_in_allocation (Default: scenario_group_name)
...
%macro corew_synth_sample_generation(inSamplingFrame =
                                    ...
                                    , scenario_var = scenario_group_name
   ...
   %do seg=1 %to &num_segments.;
      ...
      */
      ...
      %let quoted_csv_allocvar_list = %sysfunc(prxchange(s/(\w+)/"$1"/i, -1, &unquoted_csv_allocvar_list.));
      ...
      %let sql_join_csv_allocvar_list = %sysfunc(prxchange(s/(\w+)/a.$1=b.$1/i, -1, %bquote(&unquoted_csv_allocvar_list.)));
