...
/**
   \file
\anchor core_rest_get_points

   \brief   Gets data about classification points.
   ...
   \param [in] itemLimit:                Maximal number of items (points) there are in a server response.
                                            Default: 1000.
   \param [in] maxIter:                  Maximal number of requests sent.
   ...
   \details
      This macro returns named tree paths comprising classification points. The list of reported points can be narrowed down
      by using <filter>.
      This macro sends multiple requests getting <itemLimit> of points at a time. The maximal number of requests sent is
      <maxIter>.

      Data returned:
         - key: classification point key
         - namedTreeKey: named tree key
         - path: path to a named tree element
   ...
   2) Run the macro to get output table WORK.points
   \code
      %core_rest_get_points(  filter = in(key,'00000000-0000-0000-0000-000000000000','11111111-1111-1111-1111-1111111111111');
                              , outds=work.points
...
%macro core_rest_get_points(  host =
   ...
   %if %sysevalf(%superq(outds) eq, boolean) %then %do;
      %let outds=&tmplib..__points;
   %end;

   %rsk_delete_ds(&outds.);

   %let __pt_x_path_tmp = %rsk_get_unique_dsname(&tmplib.);
   %let __root = %rsk_get_unique_dsname(&tmplib., exclude = &__pt_x_path_tmp.);

   %core_set_base_url(host=&host., server=&server., port=&port.);
   %local requestUrl;
   %let fields = key,localizedPointPaths,sourceSystemCd,key;
   ...
   %do %while (&__chcnt. < &maxiter. and not &leave.);
      ...
      %let requestUrl = &baseUrl/&server./classifications/points?fields=%superq(fields);
      ...
      %core_rest_request( url = &requestUrl.
                           ...
                           , parser = sas.risk.cirrus.core_rest_parser.coreRestPoints
                           , outds = &tmplib..&__pt_x_path_tmp.
                           , arg1 = &tmplib..&__root.
      ...
      %if %upcase(&debug.) = TRUE %then %rsk_check_ds_sanity(&tmplib..&__pt_x_path_tmp.,
                                                               key namedTreeKey, check=KEY KEY,
                                                               card=0+, errflg = &errflg.);
      %if %sysevalf(%superq(&errflg) eq 1, boolean) %then %goto exit;

      %if (&__chcnt. eq 1) %then %do;
         data &outds.;
            set &tmplib..&__pt_x_path_tmp.;
         run;
      %end;
      %else %do;
         proc append base=&outds. data=&tmplib..&__pt_x_path_tmp.;
         quit;
      %end;
   ...
   %else %do;
      %rsk_delete_ds(&tmplib..&__pt_x_path_tmp.);
