%macro corew_run_model(solution =
   ...
   %if &synth_table_exists. or &elim_table_exists.  %then %do;
      ...
      /* Delete the full portfolio table if it already exists */
      %core_cas_drop_table(cas_session_name = &casSessionName.
                           , cas_libref = &inModelCaslib.
                           , cas_table = port_full_&asOfDateFmt._&casTablesTag.);
      ...
      data &inCasLibref..port_full_&asOfDateFmt._&casTablesTag. (promote=yes);
         length synthetic_instrument_flg varchar(3) forecast_interval varchar(32);
         ...
         %if (%sysevalf(%superq(forecastTimeInterval) ne, boolean)) %then %do;
            forecast_interval = "&forecastTimeInterval.";
         %end;
      run;
   ...
   %else %if(&engine. = RE) %then %do;
      ...
      */
      ...
      /*IF reParametersOverrideDs has been provided, append the override information*/

      %if "&reParametersOverrideDs." ne "" %then %do;

         %rsk_append( base = re_params
                    , data = &inModelCasLib..&reParametersOverrideDs.
                    );
      %end;

      /* Run the RE model */
      %corew_run_re_model(solution = &solution.
                        , host = &host.
                        , port = &port.
                        , logonHost = &logonHost.
                        , logonPort = &logonPort.
                        ...
                        , outReParametersDs = &outReParametersDs.
                        , asOfDate = &asOfDate.
      ...
      %if "&outReParametersDs." ne "" %then %do;
         ...
         data _null_;
            set &inModelCasLib..&outReParametersDs.;
            where upcase(name) eq "RERUNPIPELINEFLAG";
            call symputx(name, upcase(value), "L");
         run;
   ...
   /*If the pipeline did not execute - there are no CR results to promote.*/
	%if ("&reRunPipelineFlag" eq "Y") OR ("&reRunPipelineFlag" eq "") %then %do;
