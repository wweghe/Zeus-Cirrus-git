...
%macro core_krm_create_yc_data(rfm_variable_table=,
   ...
   %local temp_table_to_delete var_tbl_col_name_len assct_tbl_col_members_len assct_tbl_col_name_len
          max_num_factor_processed num_factors num_factors_processed n_ds;
   ...
   %do %while (&num_factors_processed < &num_factors);
      %if &n_ds = 1 %then %let _out_ds_= &output_yc_data;
      ...
      %if &Obs < &num_factors %then %do;
         data _null_;
            set _yc_factors(keep=yield_curve_nm factor) end=eof;
            where yield_curve_nm = "%core_get_values(ds=_yc_factors, column=yield_curve_nm, firstObs=&Obs, Obs=&Obs)";
            retain found 0 Obs &obs; 
            if factor = "%core_get_values(ds=_yc_factors, column=factor, firstObs=&Obs, Obs=&Obs)" then found=1;
            else if found = 1 then Obs + 1;
            if eof then call symputx('Obs', Obs);
         run;
      %end;

      data &_out_ds_;
         if 0 then set &krmdb_libref..YC_DATA;
         set &rfm_history_data(keep=date %core_get_values(ds=_yc_factors, column=factor, firstObs=&firstObs, Obs=&Obs));
         array his_val{*} %core_get_values(ds=_yc_factors, column=factor, firstObs=&firstObs, Obs=&Obs);;  
         ...
         if _n_ = 1 then do;
            dcl hash factors(dataset:"_yc_factors(keep=yield_curve_nm factor valueType maturityType MAT_TERM MAT_TERM_U PMT_FREQ PMT_FREQ_U day_count CouponRate maturityInYear firstobs=&firstobs obs=&obs");
...
/*          data_dt = dhms(input(date, yymmdd10.),0,0,0); */
      ...
      %if &n_ds > 1 %then %do;
         proc append base=&output_yc_data data=&_out_ds_;
         run;
      %end;
