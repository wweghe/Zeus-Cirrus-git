...
\anchor core_rest_create_analysisdata
   ...
   \param [in] allowDataAppend Adds a data definition property to allow append data to analysis data linked to the data definition
   \param [in] allowDataDelete Adds a daata definition property to allow deletion of analysis data linked to the data definition
   ...
   |------------------------------|----------------------------------------|--------------------|---------------------|-------------------------|---------------------|-----------------|-----------------------------|-----------------------|-------------------|-----------|-------------------|------------------|----------------|------------|------------------| --------------------- |--------------|----------------|---------------------|---------------------|------------|----------------------|--------------|---------------- |---------------|--------------------|
   |Portfolio data for the base   | Portfolio <MONTH, 0, SAME, yymmddd10.> |                    | ECL                 |                         | Enrichment          | PORTFOLIO       | Portfolio schema definition | Portfolio Definition  |                   |           |                   |                  |                |            |                  | REPORTING_DT INSTTYPE | BYVARS       | null           | REPORTING_DT INSTID |                     | CREDIT     | ECL_CREDIT_PORTFOLIO | FLAT         | 2022.11         | ecl202211     | CREDIT_PORTFOLIO   |
...
%macro core_rest_create_analysisdata(host =
   ...
   %if (%sysevalf(%superq(ovrDatastoreConfig) ne, boolean)) %then %do; /* (%sysevalf(%superq(ovrDatastoreConfig) ne, boolean)) */
      ...
      %let analysisDataVarsScope = 'primarykey','filterablevars','partitionbyvars','strategytype','partitioncount','attributablevars','datadefinitionname','datadefinitiondesc','targettablename','analysisdataname','analysisdatadesc','defaultindexlist','indexlist','constraintenabledflg';
   ...
   data _null_;
      ...
      length alltogether $10000.;
      alltogether=upcase(catx(' ',primaryKey,filterableVars,partitionByVars,attributableVars,segmentationVars,projectionVars));
      call symputx("alltogether",alltogether,"L");
      call symputx("primaryKey",primaryKey,"L");
      call symputx("filterableVars",filterableVars,"L");
      call symputx("segmentationVars",segmentationVars,"L");
      call symputx("projectionVars",projectionVars,"L");
      call symputx("partitionByVars",partitionByVars,"L");
      call symputx("strategyType",strategyType,"L");
      call symputx("partitionCount",partitionCount,"L");
      call symputx("attributableVars",attributableVars,"L");
      call symputx("dataDefinitionName",dataDefinitionName,"L");
      call symputx("dataDefinitionDesc",dataDefinitionDesc,"L");
      call symputx("targetTableName",targetTableName,"L");
      call symputx("analysisDataName",analysisDataName,"L");
      call symputx("analysisDataDesc",analysisDataDesc,"L");
      call symputx("defaultIndexList",defaultIndexList,"L");
      call symputx("indexList",indexList,"L");
      call symputx("constraintEnabledFlg",constraintEnabledFlg,"L");
      call symputx("dataCategoryCd",dataCategoryCd,"L");
      call symputx("dataType",dataType,"L");
      call symputx("dataSubCategoryCd",dataSubCategoryCd,"L");
      call symputx("riskTypeCd",riskTypeCd,"L");
      call symputx("schemaTypeCd",schemaTypeCd,"L");
      call symputx("businessCategoryCd",businessCategoryCd,"L");
   run;

   %let alltogether=%trim(&alltogether.);
   ...
   %if (%sysevalf(%superq(classification_point_key) eq, boolean)) %then %do;
      %if (%sysevalf(%superq(data_definition_key) ne, boolean)) %then %do;
         %let &outSuccess. = 0;
         %core_rest_get_classific_point(server = riskCirrusObjects
                                       , authMethod = &authMethod.
   ...
   data _null_;
     ...
     %if("&allowDataAppend." eq "Y") %then
       str = ',"allowDataAppend": true'; put str;
     %if("&allowDataDelete." eq "Y") %then
       str = ',"allowDataDelete": true'; put str;
         put ',"columnInfo": [';

         %if(%sysevalf(%superq(uniqueTogether) eq, boolean)) %then %do;
   ...
   %if ( %upcase("&locationType.") eq "LIBNAME" ) %then %do;
      ...
      %if ( %rsk_get_lib_engine(&location_tmp.) ne CAS ) %then %do; /* libname BASE v9 */
         %if (%sysevalf(%superq(inputDataFilter) ne, boolean) or %upcase("&enableResultAttr.") eq "Y") %then %do;
            ...
            %if ( %rsk_attrn(WORK."&filename."n, nobs) = 0) %then %do;
