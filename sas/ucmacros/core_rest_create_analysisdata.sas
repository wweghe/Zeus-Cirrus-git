...
\anchor core_rest_create_analysisdata
   ...
   \param [in] mart_aggregation_config_ds Reportmart Aggregation Config input table. 
...
%macro core_rest_create_analysisdata(host =
   ...
   %local requestUrl
      ...
      cycle_name
      analysis_run_name
      analysis_run_object_id
      genMVar
      genLen
      varsOfVars
      cas_table_exists
      promoted_filename
      TotAggregationConfig
    ;
   ...
   /* Flag to mark whether intermediate temporary libname is SPRE or CAS */
   %let location_tmp = SPRE;
   ...
   %let TotAggregationConfig = 0;
   %if %sysevalf(%superq(mart_aggregation_config_ds) ne, boolean) %then %do;
      %if(%rsk_dsexist(&mart_aggregation_config_ds.)) %then %do;
         data dummyGroupBy;
            set &mart_aggregation_config_ds.(where=(upcase(schema_Name)=upcase("&schemaName.")
               and upcase(schema_Version)=upcase("&schemaVersion."))) end = last;
            output;
            if last then do;
               weight_var="";
               alias="";
               var_name = "_rdSrcInstID_";
               var_scope = "GROUP_BY";
               output;
            end; 
         run;
         data _null_;
            length
               ...
               alias      $32.
            ;
            set dummyGroupBy end = last;
            /* Create JSON string */
            aggrDef = catx(", ", cats('"key": ', _N_)
                                 , cats('"var_name": "', var_name, '"')
                                 , cats('"var_scope": "', var_scope, '"')
                           );
            if not missing(weight_var) then 
               aggrDef = cats(aggrDef, ", ", cats('"weight_var": "', weight_var, '"'));
            
            if not missing(alias) then
               aggrDef = cats(aggrDef, ", ", cats('"alias": "', alias, '"'));
                           
            call symputx(cats("aggrDef_", _N_), aggrDef, "L");

            if last then
               call symputx("TotAggregationConfig", _N_, "L");                             
         run;
      %end;
   %end;
   ...
   data _null_;
      ...
      %if (%sysevalf(%superq(data_definition_key) eq, boolean)) %then %do;
         ...
         %if (&TotAggregationConfig. > 0) %then %do;
            str = ', "martAggregationEnabled": true';put str;

            put "       , ""martAggregationConfig"": {" @;
            put ' "items": [' @;
            %do i = 1 %to &TotAggregationConfig.;
               %if(&i > 1) %then put "," @;;
               /* Escape each quote with a backslash (i.e. "item" --> \"item\") */
               %let aggrConfigBody = {&&aggrDef_&i};
               /*%let escapedColumnInfoBody = %sysfunc(prxchange(s/([""])/\\$1/i, -1, %superq(aggrConfigBody)));*/
               /*put "%superq(escapedColumnInfoBody)" @;*/
               put "%superq(aggrConfigBody)" @;
            %end;
            put "       ]" @; /* items */
            put "       } "; /* martAggregationConfig */

         %end; %else %do; 
            str = ', "martAggregationEnabled": false';
         %end; 

         %if(%sysevalf(%superq(martFilter) ne, boolean)) %then %do;
            martFilter = symget("martFilter");;
            martFilter = prxchange('s/([""])/\\$1/i', -1, martFilter);
            str = ',"martFilter": "'|| martFilter || '"'; put str;
          %end;

         %if(%sysevalf(%superq(martConditionalAggregation) ne, boolean)) %then %do;
            martConditionalAggregation = symget("martConditionalAggregation");
            martConditionalAggregation = prxchange('s/([""])/\\$1/i', -1, martConditionalAggregation);
            str = ',"martConditionalAggregation": "'|| martConditionalAggregation || '"'; put str;
         %end;
         ...
         %else %do;
            ...
            do while (strip(scan(strip("&uniqueTogetherVars."),i,' ')) ne '');
               ...
               _attr_=strip("&measureFlagVars.");
               if indexw(_attr_,var) gt 0 then
                  put ',"measureFlag": true';
   ...
   %if ( %upcase("&locationType.") eq "LIBNAME" ) %then %do;
      ...
      %if (  (%rsk_get_lib_engine(&location.) eq CAS) or (%length(&location.) > 8) or (%index(%superq(location),%str( )) > 0) or (%sysfunc(findc(%superq(location),' ',kn)) > 0) ) %then %do; /* CAS Lib if location > 8 or < 8 but with spaces */
         libname tmpcas cas caslib="&location.";
         %let _src_lib_= tmpcas;
         %let location_tmp = CAS;
      %end;
      %else %do;
               %if ( %rsk_get_lib_engine(&location.) eq V9 ) %then %do;
                  %let _src_lib_statement_ = libname &location. "%sysfunc(pathname(&location.))";
                  %let _src_lib_= &location.;
               %end;
               %else %do;
                        /* Location point to a libname that will be passed in a view with libname deferred */
                        %if (%sysevalf(%superq(sourceLibname) eq, boolean)) %then %do;
                           %put ERROR: The Location for a non-V9 the libname must be provided via 'Location' parameter or a statement via 'sourceLibname' in datastore_config.;
                           %abort;
                        %end;
                        /* full libname statment mus be provided since it's not a sas9 libname. */
                        %let _src_lib_statement_ = &sourceLibname.;;
                        %let _src_lib_= %scan("&_src_lib_statement_.",2,' ');
                     %end;

               libname _arpvc_ "&__CORE_AR_DIR__.";
               %if &syslibrc. ne 0 %then %do;
                  %put ERROR: With LocationType=LIBNAME non CAS a PV path must be provided for macro variable '__CORE_AR_DIR__'.;
                  %abort;
               %end;
            
            &_src_lib_statement_.;
            %end;
      %if ( not %rsk_dsexist(&_src_lib_.."&filename."n) ) %then %do;
         %put ERROR: "&filename." is not available in this "&location.".;
         %abort;
      %end;

      %if ( &location_tmp. ne CAS ) %then %do; /* libname BASE v9 */
         proc sql noprint;
            create view _arpvc_._view_riskdata_&casSessionTag. as
            select *
               %if (%upcase("&enableResultAttr.") eq "Y" and (%upcase("&dataCategoryCd.") eq "RESULTS")) %then %do;
                  ...
                  , "&analysis_run_object_id." as ANALYSIS_RUN_ID length=100
                  , "&analysis_run_name." as ANALYSIS_RUN_NAME length=256
                  , "&runTypeCd." as CYCLE_RUN_TYPE length=100
               %end;
            from &_src_lib_.."&filename."n
               %if (%sysevalf(%superq(inputDataFilter) ne, boolean)) %then %do;
                  where (&inputDataFilter.)
               %end;
            using &_src_lib_statement_.
         ;quit;

         %if ( %rsk_attrn(_arpvc_."_view_riskdata_&casSessionTag."n, nobs) = 0) %then %do;
            %put WARNING: No data retrieved from &_src_lib_..;
         %end;

         %let locationType = &locationType.;
         %let location = _arpvc_;
   ...
   %if ( %upcase("&locationType.") in ("DIRECTORY") ) %then %do;
      ...
      /* validate if is V9 Base libname or a database */
      %if (%sysevalf(%superq(location) ne, boolean)) %then %do;
         /* Location points to a SO fileshare */
         %if %index(&location.,/) %then %do;
            %let _src_lib_statement_ = libname _tmp_ base "&location.";;
            %let _src_lib_= %scan("&_src_lib_statement_.",2,' ');
         %end;
         %else %do;
               %if ( %rsk_get_lib_engine(&location.) eq V9 ) %then %do;
                  %let _src_lib_statement_ = libname _loc_ "%sysfunc(pathname(&location.))";
                  %let _src_lib_= %scan("&_src_lib_statement_.",2,' ');
               %end;
               %else %do;
                     /* Location point to a libname that will be passed in a view with libname deferred */
                     %if (%sysevalf(%superq(sourceLibname) eq, boolean)) %then %do;
                        %put ERROR: The Location for a non-V9 libname must be provided via 'Location' parameter or a statement via 'sourceLibname' in datastore_config.;
                        %abort;
                     %end;
                     /* full libname statment mus be provided since it's not a sas9 libname. */
                     %let _src_lib_statement_ = &sourceLibname.;;
      ...
      %else %do;
               %if (%sysevalf(%superq(sourceLibname) eq, boolean)) %then %do;
                  %put ERROR: The Location must be provided via 'Location' parameter or a statement via 'sourceLibname' in datastore_config.;
                  %abort;
               %end;
               /* location gets info from datastore_config sourceLibname column */
               %let _src_lib_statement_ = &sourceLibname.;
               %let _src_lib_= %scan("&sourceLibname.",2,' ');
            %end;
      ...
      libname _arpvc_ "&__CORE_AR_DIR__.";
      %if &syslibrc. ne 0 %then %do;
         %put ERROR: With LocationType=DIRECTORY a PV path must be provided for macro variable '__CORE_AR_DIR__'.;
         %abort;
      %end;
      ...
      proc sql noprint;
         ...
         select *
            %if (%upcase("&enableResultAttr.") eq "Y" and (%upcase("&dataCategoryCd.") eq "RESULTS")) %then %do;
               ...
               , "&runTypeCd." as CYCLE_RUN_TYPE length=100
            %end;
         from &_src_lib_..&filename. /* V9 Base libname or a database */
         %if (%sysevalf(%superq(inputDataFilter) ne, boolean)) %then %do;
            where (&inputDataFilter.)
         %end;
            using &_src_lib_statement_.
   ...
   %else %do;
      ...
      %if ( "&marked_to_delete." eq "Y" ) %then %do;
         %if ( "&location_tmp." eq "CAS" ) %then %do;
            /* Delete table in CASUSER if exists - clean intermediate tables */
            %core_cas_drop_table(cas_session_name = &casSessionName., cas_libref = CASUSER, cas_table = &filename.);
            %core_cas_drop_table(cas_session_name = &casSessionName., cas_libref = CASUSER, cas_table = &promoted_filename.);
         %end;
         %if ( "&location_tmp." eq "SPRE" ) %then %do;
            %rsk_delete_ds(_arpvc_.&filename.);
         %end;
      %end;
   %end;
