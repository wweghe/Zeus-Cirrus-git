...
%macro core_rest_create_analysisdata(host =
   ...
   %if ( %upcase("&locationType.") eq "LIBNAME" ) %then %do;
      ...
      %if ( %rsk_get_lib_engine(&location_tmp.) ne CAS ) %then %do; /* libname BASE v9 */
         /* TODO: uncomment %if condition when riskData service provides the capability to import directly from libname */
   ...
   %if ( %upcase("&locationType.") in ("DIRECTORY") ) %then %do;
      ...
      /*%if (%sysevalf(%superq(inputDataFilter) ne, boolean) or %upcase("&enableResultAttr.") eq "Y") %then %do;*/
         ...
         %end;
         %else %do;
               /* launch libname for view input */
               &sourceLibname.;
               %let _src_lib_= %scan("&sourceLibname.",2,' ');
               %let _src_lib_statement_ = &sourceLibname.;
               libname _tmp_ "&__CORE_AR_DIR__.";
               %let location_tmp = &__CORE_AR_DIR__.;
            %end;
