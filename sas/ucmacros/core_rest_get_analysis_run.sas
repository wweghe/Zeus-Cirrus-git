...
\anchor core_rest_get_analysis_run
   ...
   \param [out] outObjParamsConfig A SAS dataset containing a mapping or an object type (objectType) to output SAS dataset name (outputDs).
      Note: objectType is the object registration's restPath
      This is useful to get a snapshot of objects exactly as they were when the analysis run was submitted.
   ...
   2) Send a Http GET request and parse the JSON response into the output table WORK.analysis_run.  Additionally:
      -Write the analysis run's parameters into the output table WORK.analysis_run_params
      -Write all model object parameters in the analysis run to the output table work.model_info
   \code

   data object_param_output_tables;
      length objectType $64 outputDs $128;
      objectType="models"; outputDs="work.model_info";
   run;
