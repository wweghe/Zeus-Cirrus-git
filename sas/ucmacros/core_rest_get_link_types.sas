...
\anchor core_rest_get_link_types
   ...
   \param [in] filter Filters to apply on the GET request when no value for key is specified. (i.e. eq(objectId,%27&linkType.%27) )
   ...
   \param [in] get_unique_link_types (Y/N). If Y, then for each link type ID (objectId) returned in the outds, only output 1 row.  This could happen if
      the same link type ID exists for both sourceSystemCd=<solution> and sourceSystemCd=RCC.  Priority is given to sourceSystemCd=<solution>. (Default: Y)
   \param [in] debug True/False. If True, debugging informations are printed to the log (Default: false)
   \param [in] logOptions Logging options (i.e. mprint mlogic symbolgen ...)
   \param [in] restartLUA. Flag (Y/N). Resets the state of Lua code submission for a SAS session if set to Y (Default: Y)
   \param [in] clearCache Flag (Y/N). Controls whether the connection cache is cleared across multiple proc http calls. (Default: Y)
   \param [out] outds Name of the output table that contains the allocation schemes (Default: work.link_types)
   \param [out] outVarToken Name of the output macro variable which will contain the access token (Default: accessToken)
   \param [out] outSuccess Name of the output macro variable that indicates if the request was successful (&outSuccess = 1) or not (&outSuccess = 0). (Default: httpSuccess)
   \param [out] outResponseStatus Name of the output macro variable containing the HTTP response header status: i.e. HTTP/1.1 200 OK. (Default: responseStatus)

   \details
   This macro sends a GET request to <b><i>\<host\>:\<port\>/riskCirrusObjects/linkTypes</i></b> and collects the results in the output table. \n
   ...
   \code
      %let accessToken =;
      %core_rest_get_link_types(outds = link_types
   ...
   | itemsCount | key                                  | objectId                            | sourceSystemCd | side1Type    | side1ObjectTypeKey                   | side2Type    | side2ObjectTypeKey                   |
   |------------|--------------------------------------|-------------------------------------|----------------|--------------|--------------------------------------|--------------|--------------------------------------|
   | 1          | b7ee05b1-2713-4a0e-80ee-6fe33d99b218 | configurationSet_configurationTable | RCC            | registration | 914bc6e8-3fd0-40b6-a20a-3c17a0093fb3 | registration | 0668db10-7f7f-475d-becd-09bada9e2e1f |
...
%macro core_rest_get_link_types(host =
                                   ...
                                   , start =
                                   , limit = 1000
                                   , get_unique_link_types = Y
                                   , outds = work.link_types
                                   , outVarToken = accessToken
                                   , outSuccess = httpSuccess
                                   , outResponseStatus = responseStatus
                                   , debug = false
   ...
   %let requestUrl = &baseUrl./&server./linkTypes;
   ...
   %core_set_rest_filter(key=&key., filter=%superq(filter), customFilter=&sscFilter., start=&start., limit=&limit.);
   ...
   %core_rest_request(url = &requestUrl.
                        ...
                        , parser = sas.risk.cirrus.core_rest_parser.coreRestLinkTypes
                        , outds = &outds.
   ...
   %if( not(&&&outSuccess..) or not %rsk_dsexist(&outds.) ) %then %do;
      %put ERROR: Unable to get the linkType;
      %abort;
   %end;

   /* If requested, ensure that each returned link type is unique.  From the request above, we could have
   a link type with the same objectId returned for both sourceSystemCd=<solution> and sourceSystemCd=RCC.  So here, for each
   link type id (objectId), output only 1 row, with priority given to sourceSystemCd=<solution>. */
   %if "&get_unique_link_types." = "Y" %then %do;

      proc sort data=&outds.; by objectId; run;

      data &outds. (drop=solution_link_type_found);
         set &outds.;
         by objectId;
         retain solution_link_type_found 0;

         if first.objectId then solution_link_type_found=0;

         if upcase(sourceSystemCd) = upcase("&solution.") then do;
            output;
            solution_link_type_found=1;
         end;
         else if last.objectId and not solution_link_type_found then output;
      run;

   %end;

%mend core_rest_get_link_types;