...
\anchor core_load_to_reportmart
   ...
   \param [in] mode Specifies the action to perform on each analysis data instance defined as input. Values: APPEND/REPLACE/REMOVE.
...
%macro core_load_to_reportmart(host =
   ...
   %if not ("&mode." in ("APPEND" "REPLACE" "REMOVE" "SKIP")) %then %do;
