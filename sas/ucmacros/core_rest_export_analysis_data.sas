...
%macro core_rest_export_analysis_data(host =
   ...
   %local oldLogOptions drop_cas_table requestUrl engine tmpFileRef filePath fileSuffix;
   ...
   /* Delete the CAS table or file if it exists before the request (and replace=true) */
   %let drop_cas_table = N;
   %if "&replace." = "true" %then %do;
      %if "&engine." eq "" or "&engine." eq "CAS" %then %do;

         %if (%upcase("&casScope.") = "GLOBAL") or (%upcase("&locationType.") = "LIBNAME" and "&casSessionId." = "") %then %do;
         ...
         %end;
         %else %if %upcase("&locationType.") = "DIRECTORY" or %upcase("&locationType.") = "FOLDER" %then %do;

            /* Assign a fileref to the file */
            %if %upcase("&locationType.") = "DIRECTORY" %then %do;
               %let filePath=&location./&fileName.;
               %if %index(&fileName., .)=0 %then
                  %let filePath=&location./&fileName..sas7bdat;
               %let tmpFileRef = %rsk_get_unique_ref(prefix=tmp, path = &filePath.);
            %end;
            %else
               %let tmpFileRef = %rsk_get_unique_ref(prefix=tmp, type=Fileref, engine=filesrvc, args=folderPath="&location." filename="&fileName.");

            /* If the file exists, remove it.  If it exists and we can't remove it, we have to error out (since riskData will error out if the request is made) */
            %if %sysfunc(fexist(&tmpFileRef.)) %then %do;
               %if %sysfunc(fdelete(&tmpFileRef.)) ne 0 %then %do;
                  %put ERROR: Failed to delete file: &location./&fileName.;
                  %abort;
               %end;
            %end;
   ...
   %if %upcase("&locationType.") = "DIRECTORY" %then %do;
      ...
      %let location=%sysfunc(urldecode(%bquote(&location.)));
      %let fileName=%sysfunc(urldecode(%bquote(&fileName.)));
      %let fileSuffix=%scan(&fileName., 2, .);

      %if "&fileSuffix."="" or %lowcase("&fileSuffix.")="sas7bdat" %then %do;

         /* This code forces a reread of contents of the libref pointing to the directory location.  This is necessary because riskData exports a SAS dataset to a library
         in its own compute session.  The library contents in the client compute session sometimes take a while to auto-refresh (sometimes like 30 seconds).
         In this case, the exported dataset in the riskData session might not be found by subsequent librefs pointing to that directory in the client session.
         This code forces the client session to reread the relevant libref contents so that it is guaranteed to be found in the client session in subsequent librefs. */
         %let tmpLibref = %rsk_get_unique_ref(prefix = tmp, type = lib, path = &location.);
         proc sql noprint;
            select count(*) into :memberRowCount from sashelp.vmember
            where libname=upcase("&tmpLibref") and memname=upcase("&fileName.");
         quit;
         libname &tmpLibref. clear;
