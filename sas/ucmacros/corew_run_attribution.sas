%macro corew_run_attribution(solution =
   ...
   %local   inCasLibref outCasLibref
            i row_list seq_no Totrows asOfDateFmt scen_override_varlist pmx_override_varlist cf_override_varlist valuedata_override_varlist
            from_scenario_name
      ;
   ...
   /***********************************************************************/
   /* Check for PMX/Cashflow/ValueData Attribution Factors if applicable    */
   /***********************************************************************/
   data work.pcvcheck2;
      set &inSasAttributionRunConfig.;
   ...
   %if %rsk_attrn(work.pcvcheck2, nlobs) NE 0 %then %do;

	   /*Create param structure for reuse - always use previous as baseline*/
	  data &inModelCasLib..base_re_params;
	      set &inModelCasLib..prev_re_params_&asOfDateFmt._&casTablesTag.;
	      where name in ("REPARAMETERMATRICES" "RECASHFLOWS" "REVALUEDATA");
	   run;
   ...
   data _null_;
      ...
      /* Set PMX parameters */
      if(pmx_switch_flg = "Y") then do;
        
         if trim(cust_re_params_pmx_table) ne "" then do;
	    /* Use custom period pmx */
	     call symputx(cats("pmx_table_", put(_N_, 8.)), cust_re_params_pmx_table, "L");
         end;
	 else do;
	     call symputx(cats("pmx_table_ ", put(_N_, 8.)), "BASE", "L");
         end;
             
      end;
      else do;

         /* Use previous period pmx and override variables */
         if (upcase(attributionType) = "PMXTABLE") then do;
            call symputx(cats("pmx_table_ ", put(_N_, 8.)), cust_re_params_pmx_table, "L");
         end;
         else do;
            call symputx(cats("pmx_table_ ", put(_N_, 8.)), "BASE", "L");
      ...
      /* Set Cashflow parameters */
      if(cf_switch_flg = "Y") then do;

         if trim(cust_re_params_cf_table) ne "" then do;
             /* Use custom period cashflow */
             call symputx(cats("cf_table_", put(_N_, 8.)), cust_re_params_cf_table, "L");    
         end;
         else do;
            call symputx(cats("cf_table_ ", put(_N_, 8.)), "BASE", "L");
         end;

      end;
      else do;

         /* Use previous period cashflow and override variables */
         if (upcase(attributionType) = "CASHFLOWTABLE") then do;
            call symputx(cats("cf_table_", put(_N_, 8.)), cust_re_params_cf_table, "L");
	 end;
	 else do;
	    call symputx(cats("cf_table_ ", put(_N_, 8.)), "BASE", "L");
       ...
       /* Set ValueData parameters */
      if(valuedata_switch_flg = "Y") then do;

         if trim(cust_re_params_vd_table) ne "" then do;
            /* Use custom period valuedata */
            call symputx(cats("valuedata_table_", put(_N_, 8.)), cust_re_params_vd_table, "L");
         end;
         else do;
            call symputx(cats("valuedata_table_ ", put(_N_, 8.)), "BASE", "L");
         end;

      end;
      else do;

         /* Use previous period valuedata and override variables */
         if (upcase(attributionType) = "VALUEDATATABLE") then do;
            call symputx(cats("valuedata_table_", put(_N_, 8.)), cust_re_params_vd_table, "L");
         end;
         else do;
            call symputx(cats("valuedata_table_ ", put(_N_, 8.)), "BASE", "L");
   ...
   */
   %do i=1 %to &TotRuns.;
      ...
      %let seq_no = &&run_sequence_no_&i..;
         ...
         /*******************************************/
         /* Perform PMX/CF/ValueData Data overrides */
         /*******************************************/

         /* Delete the re_params CAS table if it already exists */
         %core_cas_drop_table(cas_session_name = &casSessionName.
                              , cas_libref = &inModelCaslib.
                              , cas_table = re_params_&asOfDateFmt._&casTablesTag._&seq_no.);

          %if %rsk_attrn(work.pcvcheck2, nlobs) NE 0 %then %do;

		  data  &inCasLibref..re_params_&asOfDateFmt._&casTablesTag._&seq_no. (promote=yes)
		       &inCasLibref.._re_params_map_failure_;

		    set &inModelCasLib..base_re_params;

		    /***PMX***/
		    %if ("&&pmx_table_&i.." ne "BASE") %then %do;

		      if upcase(name) = "REPARAMETERMATRICES" then do;
		    ...
		    /****CF***/
		    %if ("&&cf_table_&i.." ne "BASE") %then %do;

		      if upcase(name) = "RECASHFLOWS" then do;
		    ...
		    /****ValueData***/
		    %if ("&&valuedata_table_&i.." ne "BASE") %then %do;

		      if upcase(name) = "REVALUEDATA" then do;

			  value = "&&valuedata_table_&i..";
      ...
      %if &attributionType. eq CIRRUS %then %do;
         ...
         %corew_run_model(host = &host.
                           ...
                           %if %rsk_attrn(work.pcvcheck2, nlobs) NE 0 %then %do;
                              , reParametersOverrideDs = re_params_&asOfDateFmt._&casTablesTag._&seq_no.
                           %end;
