 ...
 %macro core_krm_upload_market_data(ds_in                = ,
    ...
    %else %do;
        %if %rsk_varexist(DS=&ds_in, VAR=DATA_DT) %then %do; /*Market history data*/
           %local data_dts dt i;
           proc sql noprint;
              select distinct(data_dt) format=DATETIME22.3 into :data_dts separated by " "
              from &ds_in;
           quit;
           %do i= 1 %to &sqlobs;
              ...
              proc sql ;
                 ...
                 where %if &num_ids gt 1 %then %do;
                       ...
                       %end;
                       %else %do;
                          upcase(&id_vars) in (%upcase(%unquote(%core_get_values(ds=_history_data, column=&id_vars, dlm=%str(,), quote=single))))
                       %end;
                       and
                       data_dt = "&dt"dt
                       %if not %core_is_blank(replace_update_where) %then %do;
        ...
        %else %do; /*Marekt definition data*/
           ...
           proc sql ;
              ...
              where %if &num_ids gt 1 %then %do;
                    ...
                    %end;
                    %else %do;
                       upcase(&id_vars) in (%upcase(%unquote(%core_get_values(ds=&ds_in, column=&id_vars, dlm=%str(,), quote=single))))
                    %end;
                    %if not %core_is_blank(replace_update_where) %then %do;
                       and %unquote(&replace_update_where)
                    %end;
