...
/**
   \file
\anchor core_rest_get_flow_definitions

   \brief   Retrieve the flow definition(s) registered in SAS Process Orchestration
   ...
   \param [in] flow_id Id of the flow definition that is fetched with this REST request. If no Id is specified, the records are fetched using filter parameters.
   ...
   \param [out] outds Name of the output table that contains the flow definitions(s) (Default: flow_definitions)
   \param [out] outds_details (optional) Name of the output table that contains the task details of the flow definitions(s)
   \param [out] outVarToken Name of the output macro variable which will contain the access token (Default: accessToken)
   \param [out] outSuccess Name of the output macro variable that indicates if the request was successful (&outSuccess = 1) or not (&outSuccess = 0). (Default: httpSuccess)
   \param [out] outResponseStatus Name of the output macro variable containing the HTTP response header status: i.e. HTTP/1.1 200 OK. (Default: responseStatus)

   \details
   This macro sends a GET request to <b><i>\<host\>:\<port\>/processOrchestration/flows</i></b> and collects the results in the output table. \n
   ...
   \date    2023
*/
%macro core_rest_get_flow_definitions(host =
   ...
   %if(not &&&outSuccess..) %then %do;
      %put ERROR: The request to get the flow definition(s) was not successful.;
   ...
   %if (%rsk_dsexist(&items.)) %then %do; /* logic when using a filter */
      ...
      %if(%sysevalf(%superq(outds_details) ne, boolean)) %then %do; /* if requested: go deeper and retrieve detailed information for each flow definition */
      
         proc sql noprint;
            select id into :flow_id_list separated by " "
            from &items.
            ;
         quit;
         
         %if(%sysevalf(%superq(flow_id_list) ne, boolean)) %then %do;
            %let TotRequests = %sysfunc(countw(&flow_id_list, %str( )));
            %do i = 1 %to &TotRequests.;
               %let flowId = %scan(&flow_id_list., &i., %str( ));
               
               /* Delete temporary table if it exists */
               %if (%rsk_dsexist(__tmp_flowDefinitions_&i.__)) %then %do;
                  proc sql;
                     drop table __tmp_flowDefinitions_&i.__;
                  quit;
               %end;
               ...
               %core_rest_get_flow_definitions(host = &host.
                                            ...
                                            , flow_id = &flowId.
                                            , outds = __tmp_flowDefinitions_&i.__
               ...
               %if (%rsk_dsexist(__tmp_flowDefinitions_&i.__)) %then %do;
                  ...
                  %rsk_append(base = &outds.
                           , data = __tmp_flowDefinitions_&i.__
               ...
               proc datasets library = work
                             memtype = (data)
                             nolist nowarn;
                  delete __tmp_flowDefinitions_&i.__
                         __tmp_taskDetails_&i.__
                         ;
               quit;
               
            %end; /* %do i = 1 %to &TotRequests.; */
         %end; /* %if(%sysevalf(%superq(flow_id_list) ne, boolean)) %then %do; */
   ...
   %end;
   %else %if (%rsk_dsexist(&tasks.)) %then %do; /* logic when using flow_id */
      ...
      %if(%sysevalf(%superq(outds_details) ne, boolean)) %then %do;
         proc sql;
            create table &outds_details.(drop=ordinal_root ordinal_tasks ordinal_source) as
