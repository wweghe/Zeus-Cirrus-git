...
\anchor core_run_rules
   ...
   \param [out] outCasLib Name of the library to which the data will be passed. Default is CAS Public.
...
%macro core_run_rules(ds_rule_def =
   ...
   data
      &outLibref..&ds_out_summary. (drop = rule_primary_key rule_primary_key)
      %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
         &outLibref..&ds_out_details. (drop = rule_weight rule_match_cnt total_row_cnt)
      %end;
      ;
   ...
   %do source_key = 1 %to &tot_sources.;
      ...
      %do multi_aggr_key = 1 %to &&tot_multi_aggr_&source_key..;
         ...
         %if (&useCasLib. = N) %then %do;
            proc sql;
               create table &outLibref.._tmp_maggr_&source_key._&multi_aggr_key. as
         %end;
         %else %do;
            proc fedsql sessref=&casSessionName.;
               create table &outcaslib.._tmp_maggr_&source_key._&multi_aggr_key. as
         %end;
               select
      ...
      %if(&&tot_single_aggr_&source_key.. > 0) %then %do;
         %if (&useCasLib. = N) %then %do;
             proc sql;
                create table &outLibref.._tmp_saggr_&source_key. as
         %end;
         %else %do;
             proc fedsql sessref=&casSessionName.;
                create table &outCasLib.._tmp_saggr_&source_key. as
         %end;          
                select
                   /* Loop through all group-by aggregations */
      ...
      %if(&&tot_detail_rules_&source_key.. > 0) %then %do;
         ...
         data
            &outLibref.._tmp_dtl_rules_summary_&source_key.
            ...
            %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
               &outLibref.._tmp_dtl_rules_details_&source_key.
                  (keep = __execution_dttm
                          __source_table
                          __source_table_desc
                          __rule_id
         ...
         %if (&useCasLib. = Y) %then %do;
            length
               __execution_dttm 8.
               __source_table VARCHAR(100)
               __source_table_desc VARCHAR(256)
               __rule_id VARCHAR(100)
               __rule_name VARCHAR(100)
               __rule_desc VARCHAR(256)
               __rule_reporting_lev1 VARCHAR(1024)
               __rule_reporting_lev2 VARCHAR(1024)
               __rule_reporting_lev3 VARCHAR(1024)
               __rule_primary_key VARCHAR(4096)
               __rule_condition_txt VARCHAR(10000)
               __rule_message_txt VARCHAR(4096)
               __rule_weight 8.
               __rule_match_cnt 8.
               __total_row_cnt 8.
            ;
          ...
          %end;
            ...
            %if(&&tot_single_aggr_&source_key.. > 0) %then %do;
               %if(%rsk_attrn(__tmp_source_ds__, nobs)) %then %do;
                  if _N_ = 1 then
               %end;
                  set  &outLibref.._tmp_saggr_&source_key.;
            %end;
            ...
            %do rule_detail_key = 1 %to &&tot_detail_rules_&source_key..;
               if(&&dtl_condition_txt_&source_key._&rule_detail_key..) then do;
                  ...
                  %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
                     ...
                     output &outLibref.._tmp_dtl_rules_details_&source_key.;
                  %end;
            ...
            if last then do;
               %do rule_detail_key = 1 %to &&tot_detail_rules_&source_key..;
                  ...
                  __total_row_cnt = _N_;
                  output &outLibref.._tmp_dtl_rules_summary_&source_key.;
               %end;
            end;
         run;

         %if (&useCasLib. = Y) %then %do;
            %rsk_varlist_nm_only(ds = &outLibref.._tmp_dtl_rules_summary_&source_key.
                              , exception = RULE_MATCH_CNT TOTAL_ROW_CNT
                              , DLM =%str(,)
                                 );
            proc fedsql sessref=&casSessionName.;
               create table "&outCasLib."."_tmp_dtl_rules_summary_&source_key." {options replace=true} as
               select &VARLIST_NM.
                     , sum(rule_match_cnt) as rule_match_cnt
                     , sum(total_row_cnt) as total_row_cnt
               from "&outCasLib."."_tmp_dtl_rules_summary_&source_key."
               group by &VARLIST_NM.
            ;quit;
         %end;

         %if(%sysevalf(%superq(ds_out_summary) ne, boolean)) %then %do;
                ...
                %end;
                   ;
                   set &outLibref..&ds_out_summary. &outLibref.._tmp_dtl_rules_summary_&source_key.;
                run;
         %end;
         ...
         %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
                ...
                %end;
                   ;
                  set &outLibref..&ds_out_details. &outLibref.._tmp_dtl_rules_details_&source_key.;
                run;
         %end;
         ...
         %if (&useCasLib. = Y) %then %do;
            %core_cas_drop_table(cas_session_name = &casSessionName.
                              ...
                              , cas_table = _tmp_dtl_rules_summary_&source_key.);
            %core_cas_drop_table(cas_session_name = &casSessionName.
                              , cas_libref = &outCasLib.
                              , cas_table = _tmp_dtl_rules_details_&source_key.);
         %end;
         %else %do;
                  proc datasets library = &outLibref.
                           memtype = (data)
                           nolist nowarn;
                           delete
                                 _tmp_dtl_rules_summary_&source_key.
                                 _tmp_dtl_rules_details_&source_key.
                              ;
                  quit;
            %end;
      ...
      %do rule_aggr_key = 1 %to &&tot_aggr_rules_&source_key..;
         ...
         data &outLibref.._tmp_aggr_rules_summary_&source_key._&rule_aggr_key.
               (keep = __execution_dttm
                       __source_table
                       __source_table_desc
                       __rule_id
            ...
            %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
               &outLibref.._tmp_aggr_rules_details_&source_key._&rule_aggr_key.
                  (keep = __execution_dttm
                          __source_table
                          __source_table_desc
                          __rule_id
            ...
            if(&&aggr_condition_txt_&source_key._&rule_aggr_key..) then do;
               ...
               %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
                  ...
                  /* Write record to output detail report table */
                  output &outLibref.._tmp_aggr_rules_details_&source_key._&rule_aggr_key.;
               %end;
            ...
            if last then do;
               __total_row_cnt = _N_;
               output &outLibref.._tmp_aggr_rules_summary_&source_key._&rule_aggr_key.;
            end;
         run;
         ...
         %if(%sysevalf(%superq(ds_out_summary) ne, boolean)) %then %do;
                ...
                %end;
                   ;
                   set &outLibref..&ds_out_summary. &outLibref.._tmp_aggr_rules_summary_&source_key._&rule_aggr_key.;
                run;
         ...
         %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
                ...
                %end;
                   ;
                   set &outLibref..&ds_out_details. &outLibref.._tmp_aggr_rules_details_&source_key._&rule_aggr_key.;
                run;
