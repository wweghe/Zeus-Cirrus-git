         ...
         data
            ...
            %do rule_detail_key = 1 %to &&tot_detail_rules_&source_key..;
               if(&&dtl_condition_txt_&source_key._&rule_detail_key..) then do;
                  ...
                  %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
                     ...
                     output _tmp_dtl_rules_details_&source_key.;
                  %end;
            ...
            if last then do;
               %do rule_detail_key = 1 %to &&tot_detail_rules_&source_key..;
                  ...
                  __total_row_cnt = _N_;
                  output _tmp_dtl_rules_summary_&source_key.;
               %end;
            end;
         run;

         /* Append summary results to output */
         proc append data = _tmp_dtl_rules_summary_&source_key.
                     base = &ds_out_summary. force nowarn;
         run;

         %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
            /* Append detailed results to output */
            proc append data = _tmp_dtl_rules_details_&source_key.
         ...
         data _tmp_aggr_rules_summary_&source_key._&rule_aggr_key.
            ...
            if(&&aggr_condition_txt_&source_key._&rule_aggr_key..) then do;
               ...
               %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
                  ...
                  /* Write record to output detail report table */
                  output _tmp_aggr_rules_details_&source_key._&rule_aggr_key.;
               %end;
            ...
            if last then do;
               __total_row_cnt = _N_;
               output _tmp_aggr_rules_summary_&source_key._&rule_aggr_key.;
            end;
         run;

         /* Append summary results to output */
         proc append data = _tmp_aggr_rules_summary_&source_key._&rule_aggr_key.
                     base = &ds_out_summary. force nowarn;
         run;

         %if(%sysevalf(%superq(ds_out_details) ne, boolean)) %then %do;
            /* Append detailed results to output */
            proc append data = _tmp_aggr_rules_details_&source_key._&rule_aggr_key.
                        base = &ds_out_details. force nowarn;
            run;
         %end;
