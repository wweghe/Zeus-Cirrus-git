...
%macro corew_task_runner(host =
   ...
   %do i_run = 1 %to &TotRuns.;
      ...
      %if ( "&task_run_type." eq "report" ) %then %do;
         ...
         %if(%rsk_fileexist(&targ_folder./&report_file.)) %then
            %rsk_delete_file(&targ_folder./&report_file.);
         %if(%rsk_fileexist(&targ_folder./&report_file..bak)) %then
            %rsk_delete_file(&targ_folder./&report_file..bak);
