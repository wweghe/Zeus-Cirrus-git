 %macro corew_model_post_process(sourceSystemCd =
                                ...
                                , analysisRunKey =
                                , inResults =
                                , inResults_prev_period =
                                , ds_in_map_movement =
                                , ds_in_calculation_config =
                                , ds_in_aggregation_config =
                                , ds_in_dependency_config =
                                , ds_in_datastore_config = /* provide due to CRD ADO instance is not available at this stage */
                                , scenario_selection =
                                , out_calc_config_ds_name =
                                , outResults =
                                , outModelCasLib =
                                , asOfDate =
                                , solutionRootFolder =
   ...
   %if %sysevalf(%superq(out_calc_config_ds_name) =, boolean) %then
      %let out_calc_config_ds_name = prev_period_calcs;
   ...
   %if &solutionCadence. eq %str() %then %do;
   ...
   /* ********************************************************* */
   /*     Retrieve the previous period vars and calculations    */
   /* ********************************************************* */

   /* Check if previous period was provided */
   %if %sysevalf(%superq(inResults_prev_period) ne, boolean) %then %do;
      %corew_process_calc_config(server = riskData
                              , authMethod = bearer
                              , solution = &solution.
                              , ds_in_curr_period = &inResults.
                              , ds_in_prev_period_key = &inResults_prev_period. /* wait an object key */
                              , ds_in_calculation_cfg = &ds_in_calculation_config.
                              , ds_in_aggregation_cfg = &ds_in_aggregation_config.
                              , map_type = &mart_table_name. /* e.g. CREDIT_RISK_DETAIL */
                              , analysis_type = *
                              , scenario_selection = &scenario_selection.
                              , ds_out = &out_calc_config_ds_name.
                              , ds_out_map = _prev_period_map_1
      ...
      /* Create the data map calculation file */
      %core_map_data(ds_in = &outCasLibref..&out_calc_config_ds_name.
                     , ds_in_map_config = _prev_period_map_1
                     , map_type = &mart_table_name.
                     , fout = fmap2
                     , ds_out = &outCasLibref..&mart_table_name.
                     , ds_out_map = _prev_period_map_2
                     , outVarToken = &outVarToken.
                     );

      /* Redirect macro variable */
      %let inResults = &out_calc_config_ds_name.;
   %end;
   %else %do;
            %put NOTE: No model data provided for previous period.;
         %end;
   ...
   data &outCasLibref..
      &name_ds_in. (keep=%rsk_getvarlist(&outCasLibref..&name_ds_in.));
      ...
      %if(%sysevalf(%superq(ds_in_map_movement) ne, boolean)) %then %do;
         ...
         /* Include previous period calculation code */
         %if %sysevalf(%superq(inResults_prev_period) ne, boolean) %then %do;
            %include fmap2 / lrecl = 32000 source2;
         %end;
   ...
   %if %sysevalf(%superq(ia_data_key) ne, boolean) AND &cas_table_exists. %then %do;
      ...
      %corew_apply_indv_adjustments(solution = &solution.
                                    ...
                                    , ds_in_datastore_cfg = &ds_in_datastore_config.
