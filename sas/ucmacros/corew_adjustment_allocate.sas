%macro corew_adjustment_allocate(
   ...
   %end;
      ...
      %if %symexist(ds_in_aggregation_config) %then %do;
         %if %sysevalf(%superq(ds_in_aggregation_config) ne, boolean) %then %do;
            proc append base=WORK.__aggregation_config__ data=&ds_in_aggregation_config. force;
            run;
         %end;
      %end;
      %else %do;
      ...
      %if(&movement_id. > 1 and &tot_aggr_vars. > 0) %then %do;
         ...
         data aggregated_input_mart;
            ...
            if _N_ = 1 then do;
               declare hash hAggrMeas(dataset: "_tmp_mart_aggr_");
      ...
      %end; /* &movement_id. > 1 and &tot_aggr_vars. > 0 */
   ...
   /* *********************************************** */
      ...
      data &ds_out_modified_delta.;
         ...
         set modified_delta_trsp end = __last__;
         drop
            sequence_no
            table_id
            project_id
         ...
         if _N_ = 1 then do;
            ...
            /* Convert list of primarykey variables to comma separated quoted list of variables (needed for lookup later on) */
            %let quoted_var_list = %sysfunc(prxchange(s/\s+/%str(, )/i, -1, &primary_key.));
            %let quoted_var_list = %sysfunc(prxchange(s/(\w+)/"$1"/i, -1, %bquote(&quoted_var_list.)));
            /* Add sequence_no to the quoted var list - needed to pull unique rule_id and rule_desc values from modified delta */
            %let quoted_var_list = &quoted_var_list., "SEQUENCE_NO";

            /* Declare lookup for retrieving all the character/date/time/datetime fields and rule_id/rule_desc */
            declare hash hClass(dataset: "modified_delta(keep = &primary_key.
                                                                RULE_ID
                                                                RULE_DESC
                                                                SEQUENCE_NO
                                                                %rsk_getvarlist(&ds_in_mart., type = C)
                                                                %rsk_getvarlist(&ds_in_mart., format = &dt_fmts.|&dttm_fmts.|&tm_fmts.)
                                                         )");
            hClass.defineKey(&quoted_var_list.);
            hClass.defineData(all: "yes");
            hClass.defineDone();
