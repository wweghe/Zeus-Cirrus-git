...
\anchor corew_store_analysisdata
   ...
   \param [in] ovrRptMartConfig customized reportmart aggregation config table.
   ...
   \param [in] enableResultAttr Enables or disables a feature to add specific columns to result analysis data object (Default: 'N').
   \param [in] enableRptgFlg Enables or disables reportmart aggregation (Default: 'N').
   ...
   \param [out] rptmart_config_info Name of the output table that contains the schema info of 'reportmart_aggregation_config' (Default: ds_rpt_cfg_info).
   \param [out] rptmart_config_data Name of the output table data contains the data of the reportmart_aggregation_config configuration table (Default: ds_rpt_cfg_info).
   ...
   \code
      ...
      %corew_store_analysisdata(host = <host>
                              ...
                              , outds_configTablesInfo = work.rptmart_config_info
                              , outds_configTablesData = work.rptmart_config_data
...
%macro corew_store_analysisdata(host =
                                ...
                                , ovrRptMartConfig=
                                , rptmart_config_Info = work.ds_rpt_cfg_info
                                , rptmart_config_data = work.ds_rpt_cfg_data
                                , logSeverity = WARNING
                                , outVarToken = accessToken
                                , outSuccess = httpSuccess
                                , outResponseStatus = responseStatus
   ...
   %if &enableRptgFlg. eq Y %then %do;
      %if (%sysevalf(%superq(ovrRptMartConfig) ne, boolean)) %then %do;
         %let rptmart_config_data = &ovrRptMartConfig.;
      %end;
      %else %if (%sysevalf(%superq(configSetId) ne, boolean)) %then %do; /* Get configSet version */
               %let httpSuccess = 0;
               %let responseStatus =;
               ...
               %core_rest_get_config_table(host = &host.
                                          ...
                                          , solution = &solution.
                                          , configSetId = &configSetId.
                                          , configTableType = reportmart_aggregation_config
                                          , logSeverity = &logSeverity.
                                          , outds_configTablesInfo = &rptmart_config_Info.
                                          , outds_configTablesData = &rptmart_config_data.
                                          , outVarToken = &outVarToken.
                                          , outSuccess = &outSuccess.
                                          , outResponseStatus = &outResponseStatus.
                                          , debug = &debug.
         ...
         %end; /* %if (%sysevalf(%superq(configSetId) ne, boolean)) */
         %else %do;
                  %put ERROR: No ConfigSet or Customized reportmart config available to use. ;
                  %abort;
            %end;
      %end;
   ...
   %do k=1 %to &tot_objs;
      ...
      %core_rest_create_analysisdata(host = &host.
                                 ...
                                 %if &enableRptgFlg. eq Y %then %do;
                                    , mart_aggregation_config_ds = &rptmart_config_Data.
                                 %end;
                                    , outVarToken = &outVarToken.
                                    , outSuccess = &outSuccess.
                                    , outResponseStatus = &outResponseStatus.
