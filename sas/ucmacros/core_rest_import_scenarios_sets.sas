...
\anchor core_rest_import_scenarios_sets
   ...
   \param [in] listScenariosWeight List of the weights per scenarioId listed in parameter 'listScenariosId'. Is a numeric value and admits 'null'. (list with comma separated)
   ...
   data _null_;
      ...
      i = 1;
         do while (strip(scan(strip("&listScenariosId."),i,',')) ne '');
            ...
            if strip(scan(strip("&listScenariosWeight."),i,',')) eq "" then do;
             str =  ',"weight": null'; put str;
            end;
            else do;
                  str =  ',"weight": '||strip(scan(strip("&listScenariosWeight."),i,','))||''; put str;
            end;
            i=i+1;
