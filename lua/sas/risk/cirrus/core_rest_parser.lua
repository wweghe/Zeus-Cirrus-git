...
coreRestAnalysisRun = function(filename, output, outputParms, foutCode)
   ...
   if outputParms ~= nil then
      ...
      outParmsTable.vars.name = {type = "C", length = 200}
      outParmsTable.vars.value = {type = "C", length = 32000}
      outParmsTable.vars.objectKey = {type = "C"}
      outParmsTable.vars.objectId = {type = "C"}
      outParmsTable.vars.objectName = {type = "C"}
      outParmsTable.vars.objectCustomFields = {type = "C", length = 32000}
      outParmsTable.vars.isValid = {type = "C"}
         ...
         for jsonKey, jsonValue in pairs(value) do
            ...
            if jsonKey == "resolvedExpressions" and type(jsonValue) == "string" then
               ...
               for j, param in ipairs(resolvedExpressions.scriptParameterResults) do
                  ...
                  -- If the parameter is an object parameter, its expandedValue will be a table.  The first element in this table
                  -- is that object's JSON definition.
                  -- In theory a parameter could have multiple elements in this table (objects will only have 1),
                  -- so loop over all and add to the final table
                  if type(param.expandedValue) == "table" then
                     row.value = json:encode(param.expandedValue)
         ...
         for key, value in pairs(result) do
            for result_key, result_value in pairs(value) do
               ...
               if result_key == "results" and type(result_value) == "string" then
                  -- Transform the JSON string in Lua table --
                  local tdResult = json:decode(result_value)
                  -- print("tdResult info: " .. table.tostring(tdResult))

               for row_k, row_v in pairs(tdResult) do
                  -- print("row_k info: " .. tostring(row_k))
                  -- print("row_v info: " .. tostring(row_v))
         ...
         for jsonKey, jsonValue in pairs(value) do
            if jsonKey == "bepData" then
               ...
               for _, bepRow in ipairs(bepData.data) do
                  ...
                  row.targetDefinition = bepRow.targetDefinition
                  row.targetType = bepRow.targetType
                  ...
                  if bepRow.hasChildren == false then
                     ...
                     local absoluteValue = bepRow.horizon0
                     for h=0, intervalCount do
                        ...
                        if bepRow.targetDefinition == "PERCENTAGE" then
                           horizonRow.relativePct = bepRow[horizonRow.intervalName]
                           if horizonRow.relativePct then
                              horizonRow.relativeValue = absoluteValue * horizonRow.relativePct
                           end
                        else
                           horizonRow.relativeValue = bepRow[horizonRow.intervalName]
                           if horizonRow.relativeValue then
