...
M.generatePostgresSQLPartition=function(sourceLibname,sourceDSName,targetLibname,targetDSName,parameters,schemaName)
    ...
    local CASDSParameters = M.getCASDSParameters(sourceLibname)
    ...
    local sasPartitionCode=[[
            ...
            data @fullTargetDS@ (BULKLOAD=YES DBCREATE_TABLE_OPTS = '
                  @partitionCreateStr@
                  @primaryKeyStr@
                  @indexStr@
                  @partitionStr@
                  ') ;
            set  @fullSrcDS@ @CASDSParameters@;
            @srcWhereStmt@
           run;
        ]]
