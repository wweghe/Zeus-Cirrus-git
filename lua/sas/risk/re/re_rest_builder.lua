...
buildPipelineBody = function(pipelineJsonFile
   ...
   for i, inputRow in ipairs(sas.read_ds(configurationInputDs)) do
      ...
      elseif (inputRow.type == "valueData") then
         valueDataInputs[inputRow.parameter]=inputRow.value
      end
   end
   ...
   if (debug) then
      ...
      print(table.tostring(valueDataInputs))
   end
   ...
   for pipelineKey, pipelineVal in pairs(pipelineTable) do
      ...
      if (pipelineKey == "actionNodes") then
         ...
         for i, actionNode in ipairs(pipelineVal) do
            ...
            elseif (actionNode.type == "evaluatePortfolio") then
               ...
               actionNode["crossClassVariables"] = evaluatePortfolioInputs.reCrossClassVariables ~= "" and string.split(evaluatePortfolioInputs.reCrossClassVariables,", ") or actionNode.crossClassVariables
               actionNode["crossClassVariableSource"] = {type="portfolio"}
         ...
         for _, dataNode in ipairs(pipelineVal) do
            ...
            if (dataNode.type == "marketData") then
               ...
               if (dataNode["dataType"] == "combined") then
                  ...
                  dataNode.combinedData["interval"] = marketDataNodeInputs.reInterval ~= "" and string.lower(marketDataNodeInputs.reInterval) or dataNode.combinedData["interval"]
                  dataNode.combinedData["intervalAlignment"] = marketDataNodeInputs.reIntervalAlignment ~= "" and string.lower(marketDataNodeInputs.reIntervalAlignment) or dataNode.combinedData["intervalAlignment"]
                  local horizonCount = tonumber(marketDataNodeInputs.reCombinedHorizonCount) or dataNode.combinedData["horizonCount"]
                  dataNode.combinedData["horizonCount"] = (horizonCount>0) and horizonCount or 1
      ...
      if (pipelineKey == "configuration") then
         ...
         --Add valueData data mappings
         for vdName, vdTable in pairs(valueDataInputs) do
            local vdObjectMapping = {riskDataObject={name=vdName, type="valueData"}, table=casTableJson(configurationInputs.inputCaslib, vdTable)}
            table.insert(pipelineVal["riskDataObjectMappings"], vdObjectMapping )
         end
