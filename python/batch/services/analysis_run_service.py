...
class AnalysisRunService(RunnableService):
    ...
    def execute_action(self, config : BaseConfigRunnable, *args, **kwargs) -> Any:
        ...
        if (action == BatchRunActionEnum.DELETE):
            return self.delete(analysis_run_config = config)
        elif (action == BatchRunActionEnum.CREATE):
            return self.create(analysis_run_config = config)
        elif (action == BatchRunActionEnum.UPDATE):
            return self.update(analysis_run_config = config)
        elif (action == BatchRunActionEnum.RUN):
            return self.run(analysis_run_config = config)
    ...
    def delete(self, 
        ...
        self._batch_job_service.update_step(config = analysis_run_config)
    ...
    def run(self, 
        ...
        if wait:
            batch_step_id = self._batch_job_service.get_step_id_by_config(config = analysis_run_config)
