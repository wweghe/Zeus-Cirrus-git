...
class SolutionService:
    ...
    __STATE_KEY_SOLUTION_DETAILS : str = "solution_repository"
    __STATE_KEY_CURRENT_USER : str = "current_user"
    ...
    def __get_solution_details(self, reload : bool = False) -> Any:
        ...
        try:
            solution_details = self._state.get(self.__STATE_KEY_SOLUTION_DETAILS, None)
            ...
            if (reload or solution_details is None): 
                ...
                if (solution is not None):
                    ...
                    self._state.update(self.__STATE_KEY_SOLUTION_DETAILS, solution_details)
        finally:
            self._state.unlock()
    ...
    def __get_application_property(self, 
        ...
        config_value = getattr(solution_details.ui.application.configurationProperties, property_name).value
        has_value = (config_value is not None and len(str(config_value)) > 0)

        if (return_type in [object, SimpleNamespace]):
            if has_value:
                return json.loads(config_value, object_hook = lambda d: SimpleNamespace(**d))
            return None
    ...
    def get_current_user(self, reload : bool = False) -> None:
        ...
        try:
            current_user : SimpleNamespace = self._state.get(self.__STATE_KEY_CURRENT_USER, None)

            if (reload or current_user is None):
               current_user = self._user_repository.get_current_user()
               self._state.update(self.__STATE_KEY_CURRENT_USER, current_user)
        finally:
            self._state.unlock()
