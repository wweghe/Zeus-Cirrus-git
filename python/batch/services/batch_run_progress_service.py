...
class BatchRunProgressService:
    ...
    def __get_header_message(self):
        msg = f"Running {constants.APP_FULL_NAME} ...\n" + \
    ...
    def __get_result_message(self, 
        ...
        message = f"{str(os.getpid()):10s}{result.object_type:15s}{str(result.object_id) + ':' + str(result.source_system_cd):25s}{result.action + ' ' if result.action is not None else '':15s}{result.status.value if result.status is not None else '':15s}{result.elapsed_time_str}"
        filled_length = self.__LINE_LENGTH - len(message)
        message += " " * (filled_length if filled_length < self.__LINE_LENGTH else 1)
    ...
    def progress(self, 
        ...
        try:
            ...
            progress_key = f"{os.getpid()}{config.get_ordinal_number()}"
