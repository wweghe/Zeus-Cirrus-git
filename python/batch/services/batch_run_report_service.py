    ...
    def _write_status(self, 
        ...
        for result in batch_run_results:
            ...
            if column_map is None:
                ...
                sheet.cell(row = 1, column = column).value = "RESULT"
                sheet.cell(row = 1, column = column + 1).value = "ERROR"
                sheet.cell(row = 1, column = column + 2).value = "DURATION"

            row = self.__get_row_for_object(sheet, column_map, key_column_map = key_column_map, row_offset = row_offset)
            if (row > 0):
                
                sheet.cell(row = row, column = column).value = result.status.value if result.status is not None else ''
                sheet.cell(row = row, column = column + 1).value = result.error_message
                sheet.cell(row = row, column = column + 2).value = result.elapsed_time_str
