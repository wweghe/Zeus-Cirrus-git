...
from services.batch_run_worker import BatchRunWorker
...
class BatchRunService:
    ...
    def __init__(self, 
                ...
                batch_run_worker : BatchRunWorker,
        ...
        if (batch_run_worker is None): raise ValueError(f"batch_run_worker cannot be empty")
        ...
        self._batch_run_worker = batch_run_worker
        self._batch_logger = batch_logger
        self._batch_job_service = batch_job_service
        self._request_service = request_service


    def run(self, ctx : DefaultContext, batch_config: BatchConfig) -> List[BatchRunResult]:
        ...
        cycle_configs_async = \
            [(config, self._batch_logger.init_queue, self._request_service.reinit) for config in batch_config.cycle_configs if config.get_is_parallel()] \
            if batch_config.cycle_configs is not None else []
        analysis_run_configs_async = \
            [(config, self._batch_logger.init_queue, self._request_service.reinit)  for config in batch_config.analysis_run_configs if config.get_is_parallel()] \
        ...
        self._batch_job_service.create_steps(configs = configs + [c[0] for c in configs_async])

        if (len(configs) > 0):
            for config in configs:
                result.append(self._batch_run_worker(config, logger = self._batch_logger))

        if (len(configs_async) > 0):
            with ctx.Pool(processes = self._launch_args.max_parallel_processes) as pool:
                res_async = pool.starmap(self._batch_run_worker, configs_async)
                pool.close()
                pool.join()
                result += res_async
