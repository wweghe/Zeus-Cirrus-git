    ...
    def _resolve_parameter_expression(self, 
        ...
        if is_single_result:
            ...
            query_result_single, _ = repository.get_by_key(
                    key = key_value,
                    fields = getattr(object_query.query, "fields", None))
        else:
            filter_value = getattr(object_query.query, "filter", "")
            if (context is not None and len(str(filter_value)) > 0):
                filter_value = str(filter_value).format(**context.__dict__)
