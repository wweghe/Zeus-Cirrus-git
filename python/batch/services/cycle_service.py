...
class CycleService:
    ...
    def __set_payload_run_type(self, 
                               cycle : CirrusObject,
                               cycle_config : CycleConfig,
                               field : str) -> bool:
        ...
        runTypeCd = str(getattr(cycle_config, self.__FIELD_RUN_TYPE_CD, runType_default)).upper()
        ...
        if hasattr(cycle_config, self.__FIELD_RUN_TYPE_CD):
            ...
            cycle.set_field(self.__FIELD_RUN_TYPE_CD, runType_default)
    ...
    def __set_payload_field(self,
        ...
        if not is_set:
            is_set = self.__set_payload_run_type(cycle, cycle_config, field)
    ...
    def __create_save_payload(self,
        ...
        for attr in list(config_attributes):
            if cycle.is_field(attr, custom_field_names):
                ...
                classification = self.__create_classifications(cycle_config)
        ...
        self.__validate(cycle = cycle, cycle_config = cycle_config)
    ...
    def __start_workflow(self, 
        ...
        definitions = self._workflow_definition_repository.get_by_filter(
            filter = f"eq(name,'{workflow_template.customFields.wfDefinitionName}')")
        if (definitions is None or len(definitions) == 0):
            raise RuntimeError(f"Workflow definition " \
                               f"'{workflow_template.customFields.wfDefinitionName}' not found")
        workflow_definition = definitions[0]
        ...
        if (not cycle_updated.has_workflow_tasks()):
            return self._cycle_repository.wait_for_workflow_tasks(
                cirrus_object_key = cycle_updated.key, 
    ...
    def __transition_to_workflow_task(self,
        ...
        if (not cycle.has_workflow_tasks()):
        ...
        if (not cycle_updated.has_workflow_tasks()
            and cycle_updated.is_workflow_running()):
    ...
    def run(self, 
        ...
        cycle, _ = self.__get_cycle_from_config(cycle_config, False)
        ...
        has_tasks = cycle.has_workflow_tasks()
        is_workflow_running = cycle.is_workflow_running()
        
        if (not is_workflow_running):
            has_tasks, cycle, etag = self.__start_workflow(cycle_config, cycle, workflow_template)
        ...
        while (has_tasks and is_workflow_running):
            ...
            if has_tasks:
                ...
                for task in tasks_to_run:
                    workflow_config = self._batch_config.get_uncompleted_cycle_workflow_by_task_name(cycle_config.get_key(), task_name = task.name)
