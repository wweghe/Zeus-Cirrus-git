...
class SharedStateKeyEnum(str, Enum):
    ...
    ACCESS_SESSION : str = "access_session"
...
class BatchLogQueue:
    def __init__(self, queue : Queue) -> None:
        if (queue is None): raise ValueError(f"queue cannot be empty")
        self._queue = queue
    ...
    def close(self) -> None:
        pass #self._queue.close()

    def put_nowait(self, obj : Any) -> None:
        self._queue.put_nowait(obj)


class BatchLogQueueProxy(ProxyBase):
    _exposed_ = tuple(dir(BatchLogQueue))
