...
class AnalysisRunConfig(BaseConfigRunnable):
   
    def __init__(self, ordinal_number = 0) -> None:     
        super().__init__(
            rest_path = constants.REST_PATH_ANALYSIS_RUN,
            object_type = constants.OBJECT_TYPE_ANALYSIS_RUN,
    ...
    def validate_headers(self, 
        ...
        if self._ACTION_COLUMN_NAME not in column_other_map:
            errors.append(f"Mandatory column {repr(self._ACTION_COLUMN_NAME)} was not found in '{constants.SHEET_NAME_ANALYSIS_RUNS}' worksheet.")
        # is_parallel
        if self._IS_PARALLEL_COLUMN_NAME not in column_other_map:
            errors.append(f"Mandatory column {repr(self._IS_PARALLEL_COLUMN_NAME)} was not found in '{constants.SHEET_NAME_ANALYSIS_RUNS}' worksheet.")
