...
def usage() -> LaunchArguments:
    parser = argparse.ArgumentParser(
        prog = 'Risk Cirrus Core Batch Utility',
    ...
    parser.add_argument(
        ...
        required = False)
    parser.add_argument(
        '--auth-host', '--auth-url', '-ah', 
        help = 'Third party authentication host url for getting authentication token, e.g. http(s)://test.sas.com:80. ' \
            'If authentication url is not provided, application will derive the url based on host option',
        required = False)
    parser.add_argument(
        '--user', '-u',
    ...
    parser.add_argument(
        ...
        required = False, 
        default = os.environ.get('SAS_CLIENT_DETAILS') or 'sas.cli:')
    parser.add_argument(
        '--auth-token', '-at',
    ...
    # parser.add_argument(
    #     '--consul-token', '-ct',
    #     help = 'Consul token for client registration and usage further. Not yet supported.', 
    #     required = False)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help = f"Log format record. ' \
            'Default value: {constants.LOG_FORMAT.replace('%', '%%')}", 
...
def main():
    launch_args = usage()
    di_container = compose(launch_args)
    ...
    batch_run_service.run(batch_config)
